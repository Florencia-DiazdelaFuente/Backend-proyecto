...
app.get("/api/products/:id", async(req, res)=>{
    try {
        ...
        if (Number.isNaN(id) || id < 0) {
            return res.send({success: false, error:"El id debe ser un número válido"})
        }
...
app.post("/api/products", async(req,res)=>{
    try{
        ...
        const savedProduct = await productManager.saveProduct({title, description, price, thumbnail, code, stock})
...
app.put("/api/products/:id", async (req,res) => {
    try {
        ...
        if (Number.isNaN(id) || id < 0) {
            ...
            const updatedProduct = await productManager.update(id, {title, description, price, thumbnail, code, stock})
